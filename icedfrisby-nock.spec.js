...
describe('icedfrisby-nock', function () {
  ...
  describe('disables network connections', function () {
    ...
    frisby.create('disables network connection')
      .post('http://example.com/test')
      .intercept(nock => nock('http://example.com')
        .post('/test')
        .reply(418, { someKey: 'someValue' }))
    ...
    after(function () {
      return networkRequest
        ...
        .catch(e => {
          const expectedMessage = 'request to http://httpbin.org failed, reason: Nock: Not allow net connect for "httpbin.org:80/"'
  ...
  describe('when the test has finished', function () {
    ...
    context('and succeeded', function () {
      frisby.create('when the test has finished and succeeded')
        .post('http://example.com/test')
        .intercept(nock => nock('http://example.com')
          .post('/test')
          .reply(418, () => {
    ...
    context('and failed', function () {
      const test = frisby.create('when the test has finished and failed')
        .post('http://example.com/test')
        .intercept(nock => nock('http://example.com')
          .post('/test')
          .reply(418, () => {
  ...
  describe('conditional intercept', function () {
    it('sets up a mock which works correctly if condition true', function () {
      frisby.create(this.test.title)
        .post('http://example.com/test')
        .conditionalIntercept(true, nock => nock('http://example.com')
          .post('/test')
          .reply(418, { someKey: 'someValue' }))
        .expectStatus(418)
        .expectJSON({ someKey: 'someValue' })
    ...
    it('does not set up a mock if condition false', function () {
      frisby.create(this.test.title)
        .get('http://httpbin.org')
        .conditionalIntercept(false, nock => nock('http://httpbin.org')
          .get('/')
          .reply(418, { someKey: 'someValue' }))
        .expectStatus(200)
        .toss()
